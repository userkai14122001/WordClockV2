...
void MatrixRainEffect::update() {
    ...
    uint8_t pauseMax = (uint8_t)intensityMap(25, 5);
    ...
    for (int x = 0; x < WIDTH; x++) {
        ...
        for (int t = 0; t < (int)c.len; t++) {
            ...
            if (t == 0) {
                strip->setPixelColor(XY(x, y), makeColorWithBrightness(200, 255, 200));
            } else {
                float fade = 1.0f - (float)t / (float)c.len;
                uint8_t g = (uint8_t)(220.0f * fade * fade);
                uint8_t r = (uint8_t)(8.0f   * fade);
                strip->setPixelColor(XY(x, y), makeColorWithBrightness(r, g, r / 3));
            }
        }
    }
